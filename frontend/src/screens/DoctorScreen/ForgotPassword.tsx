...
import { useSendDoctorResetOtpMutation,useVerifyOtpDoctorMutation } from '../../slices/doctorSlice/doctorApiSlice'; // Doctor-specific API call to request OTP
import FormContainer from '../../components/patientComponents/FormContainer'; // Use Doctor's FormContainer component
...
const RequestDoctorOtpScreen: React.FC = () => {
  ...
  const [sendDoctorResetOtp, { isLoading }] = useSendDoctorResetOtpMutation();
  const [verifyDoctorOtp] = useVerifyOtpDoctorMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await sendDoctorResetOtp({email}).unwrap();
  ...
  const handleOtpVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/doctor/DoctorResetPasswordScreen', { state: { email } }); // Adjust route for doctor's reset password
    } catch (error) {
